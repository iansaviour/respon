...
<?php  
    ...
    foreach ($data as $datax) {
      if($datax['id_service']==$id_service){
        $query = "INSERT INTO $datax[outbox_table]($datax[outbox_content],$datax[outbox_date],$datax[outbox_flag]) VALUES('$username".$datax['char_pemisah']."$password','".date("Y-m-d")."','1') ";
        $result = mysqli_query($id_mysql,$query);
        if ($result) {
          $output= mysqli_insert_id($id_mysql);
          $query_upd = "UPDATE $datax[outbox_table] SET $datax[outbox_content]='HOST".$datax['char_pemisah'].$output.$datax['char_pemisah']."$username".$datax['char_pemisah']."$password' WHERE id='$output'";
